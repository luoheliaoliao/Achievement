    ...
    validatebox: {
        ...
        rules: {
            ...
            Pwd: {
                ...
                message: '密码请输入不少于8个字符且至少包括字母、数字！'
            },
            EasyPwd: {
                validator: function (value) {
                    if (value.length < 6) {
            ...
            EngNumUnderlinePoint: {
                validator: function (value) {
                    return (/^[a-zA-Z0-9\.\/_]*$/).test(value);
                },
                message: '只允许输入数字字母./以及_！'
            },
            ChnEngNum: {
                validator: function (value) {
                    return (/[A-Za-z0-9\u4e00-\u9fa5]+$/).test(value);
                },
                message: '只允许输入中文和字母和数字！'
            },
            Chn: {
                validator: function (value) {
            ...
            //},
            ProductQty: {
                validator: function (value, unitMeterType) {
            
                    if (unitMeterType.toLowerCase() == "weight" || unitMeterType == 1) {
                        if ($.validatebox.rules.ProductQty.message != '称重只允许输入有三位小数的非负数，且整数部分不能超过8位！') {
                            $.validatebox.rules.ProductQty.message = '称重只允许输入有三位小数的非负数，且整数部分不能超过8位！';
                            $.validatebox.methods.valid_load();
                        }
                        return (/^\d{1,8}$|^\d{1,8}[.]\d{1,3}$/).test(value);
                    } else {
                        if ($.validatebox.rules.ProductQty.message != '只允许输入不超过8位的非负整数！') {
                            $.validatebox.rules.ProductQty.message = '只允许输入不超过8位的非负整数！';
                            $.validatebox.methods.valid_load();
                        }
            ...
            Integer://非负整数
            {
                ...
                message: '只允许输入非负整数，且不能超过8位！'
            },
            NonzeroInteger: {
                validator: function (value) {
                    return (/^\+?[1-9][0-9]{0,7}$/).test(value);
                },
                message: '只允许输入正整数，且不能超过8位！'
            },
            NumP1: {
                validator: function (value) {
                    return (/^\d{1,8}$|^\d{1,8}[.]\d{1,1}$/).test(value);
                },
                message: '只允许输入有1位小数的非负数，且整数部分不能超过8位！'
            },
            NonzeroNumP1: {
                validator: function (value) {
                    return (/^[1-9][0-9]{0,7}$|^\d{1,8}[.]\d{1,1}$/).test(value);
                },
                message: '只允许输入有1位小数的正数，且整数部分不能超过8位！'
            },
            NumP2: {//金额都要用这个
                validator: function (value) {
                    return (/^\d{1,8}$|^\d{1,8}[.]\d{1,2}$/).test(value);
                },
                message: '只允许输入有2位小数的非负数，且整数部分不能超过8位！'
            },
            NonzeroNumP2: {
                validator: function (value) {
                    return (/^[1-9][0-9]{0,7}$|^\d{1,8}[.]\d{1,2}$/).test(value);
                },
                message: '只允许输入有2位小数的正数，且整数部分不能超过8位！'
            },
            NumP3: {
                validator: function (value) {
                    return (/^\d{1,8}$|^\d{1,8}[.]\d{1,3}$/).test(value);
                },
                message: '只允许输入有3位小数的非负数，且整数部分不能超过8位！'
            },
            NonzeroNumP3: {
                validator: function (value) {
                    return (/^[1-9][0-9]{0,7}$|^\d{1,8}[.]\d{1,3}$/).test(value);
                },
                message: '只允许输入非零的有3位小数的正数，且整数部分不能超过8位！'
            },
            RangeInteger: {
                param: null,
                validator: function (value) {
                    var integer = $.trim(value) * 1;
                    var len = $.trim(value).length;
                    var paramArray = this.param.split(",");
                    var fromNum = paramArray[0] * 1;
                    var toNum = paramArray[1] * 1;
                    if (integer < fromNum || integer > toNum || len == 0)
                        return false;
                    return (/^\d+$/).test(value);
                },
                message: '只能输入{0}~{1}的整数！'
            },
            NumPercent: {
                validator: function (value) {
                    return (/^(([0-9]|[0-9][0-9])(\.\d{1,2})?|100|100.0|100.00)$/).test(value);
                },
        ...
        methods: {
            ...
            showTip: function (target, msg) {
                var box = target;
                var tag = $(box);
                if (tag.parent("div").find(".alert").length == 0) {
                    tag.parent("div").append(' <div class="alert" style="margin-top:4px;padding-left:10px;"><i class="fa fa-warning"></i> ' + msg + '</div>');
                }
                ...
                //    var q = tag.next($(".chosen-container"));
                //    if (p&&!q) {
                //        tip.css({
                //            display: 'block',
                ...
                //        tip.css({
                //            display: 'block',
                //            left: q.offset().left + q.outerWidth(),
                //            top: q.offset().top
                //        });
                //    }
            ...
            hideTip: function (target) {
                $(target).parent("div").find(".alert").remove();
            ...
            valid_load: function () {
                ...
                controls.each(function (i, val) {
                    ...
                    $(box).unbind('focus mouseover change keyup');
            ...
            tip: function (ruleArray, box) {
                ...
                $(ruleArray).each(function (i, val) {
                    ...
                    if (rule.param != undefined) {
                        if (!rule.validator($.trim($(box).val()), rule.param)) {
                            valid = false;
        ...
        vaildfocus: function (container) {
            return this.methods.vaildfocus(container);
        }
